...
final class ExtractorClient
{
    ...
    private function mapResponse(Response $response, string $requestId): ExtractorRecognition
    {
        ...
        if ($response->serverError()) {
            ...
            if (
                $provider === MediaPlatform::TikTok->value
                && is_string($code)
                && in_array($code, [
                    'rate_limited_upstream',
                    'provider_timeout',
                ...
                throw new ExtractorException(
                    ...
                    match ($code) {
                        ...
                        'provider_response_changed' => 'TikTok temporarily returned an unsupported metadata format. Please try again later.',
                        'provider_response_too_large' => 'TikTok returned more metadata than the service accepts.',
                        default => 'TikTok metadata is temporarily unavailable.',
                    },
                );
            }
    ...
    private function isSafeTikTokUrl(string $url, mixed $variant): bool
    {
        $parts = parse_url($url);
    ...
    private function readyPlatform(string $provider, ?string $variant): ?MediaPlatform
    {
        ...
        if ($provider === MediaPlatform::TikTok->value) {
