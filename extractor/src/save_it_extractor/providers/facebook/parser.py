...
def parse_facebook_video(
    ...
    if not variants:
        raise ProviderError("no_media", "The public Facebook post does not contain extractable video.", 422, {"provider": "facebook"})
    ...
    asset = {
        "id": "asset-1", "order": 1, "type": "video", "role": "primary", "url": selected["url"],
        "thumbnail_url": poster, "mime_type": "video/mp4",
        # Post dimensions are intentionally never copied into rendition metadata.
        "width": _positive(video.get("width")), "height": _positive(video.get("height")),
        "duration_ms": _duration_ms(video), "alt_text": None,
        "variants": [{
            "url": item["url"], "mime_type": "video/mp4", "protocol": "https",
            "bitrate": item["bitrate"], "width": item["width"], "height": item["height"],
            "fps": None, "container": "mp4", "quality_label": item["quality_label"],
            "filesize": None, "is_preferred": index == 0,
...
def _story_candidates(state: Any, story_id: str) -> list[dict[str, Any]]:
    ...
    candidates: list[dict[str, Any]] = []
    attachment_keys = {
        "attachments", "attachment", "subattachments", "media",
    ...
    def attachment_walk(value: Any, allowed: bool = False) -> None:
        if isinstance(value, dict):
            ...
            for key, child in value.items():
                attachment_walk(child, allowed or key in attachment_keys)
        elif isinstance(value, list):
            for child in value:
...
def _has_delivery(video: dict[str, Any]) -> bool:
    return isinstance(video.get("videoDeliveryLegacyFields"), dict) or isinstance(video.get("videoDeliveryResponseFragment"), dict)
...
def _unique_candidates(candidates: list[dict[str, Any]], expected_video_id: str | None) -> list[dict[str, Any]]:
...
def _variants(video: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    dash_by_identity = {_media_identity(item["url"]): item for item in dash if item["kind"] == "video"}
    raw: list[dict[str, Any]] = []
    legacy = video.get("videoDeliveryLegacyFields")
    if isinstance(legacy, dict):
        for key, rank, native_quality in (("browser_native_hd_url", 2, "HD"), ("playable_url_quality_hd", 2, "HD"), ("browser_native_sd_url", 1, "SD"), ("playable_url", 1, "SD")):
            url = legacy.get(key)
            if isinstance(url, str) and is_allowed_asset_url(url):
                raw.append(_enrich_native(_candidate(url, None, rank, native_quality), dash_by_identity))
    ...
    if isinstance(progressive, list):
        for item in progressive:
            if isinstance(item, dict) and isinstance(item.get("progressive_url"), str) and is_allowed_asset_url(item["progressive_url"]):
                metadata = item.get("metadata") if isinstance(item.get("metadata"), dict) else None
                raw.append(_enrich_native(_candidate(item["progressive_url"], metadata, 0, None), dash_by_identity))

    # A declared H.264 DASH representation has stream-specific dimensions. It is
    # safe to expose as a distinct rendition; other codecs remain metadata only.
    for item in dash:
        if item["kind"] == "video" and item["codec"].startswith(("avc", "h264")):
            raw.append(_candidate(item["url"], item, 0, None))

    deduplicated: dict[str, dict[str, Any]] = {}
    for item in raw:
        identity = _media_identity(item["url"])
        deduplicated[identity] = item if identity not in deduplicated else _merge_candidate(deduplicated[identity], item)
    ...
    for adaptation in root.iter():
        ...
        for representation in adaptation:
            ...
            if not isinstance(url, str) or not is_allowed_asset_url(url):
                continue
            codec = str(representation.attrib.get("codecs") or adaptation.attrib.get("codecs") or "")
            kind = "audio" if mime == "audio" or codec.startswith("mp4a") else "video"
            if kind == "audio" and not codec.startswith("mp4a"):
                continue
            variants.append({"kind": kind, "url": url, "codec": codec, "width": _positive_attribute(representation, "width"), "height": _positive_attribute(representation, "height"), "bitrate": _positive_attribute(representation, "bandwidth")})
    return variants


def _enrich_native(candidate: dict[str, Any], dash_by_identity: dict[str, dict[str, Any]]) -> dict[str, Any]:
    dash = dash_by_identity.get(_media_identity(candidate["url"]))
    return candidate if dash is None else _merge_candidate(candidate, _candidate(dash["url"], dash, candidate["quality_rank"], candidate["native_quality"]))


def _merge_candidate(left: dict[str, Any], right: dict[str, Any]) -> dict[str, Any]:
    winner = left if _rank(left) <= _rank(right) else right
    merged = {**winner, "width": left["width"] or right["width"], "height": left["height"] or right["height"], "bitrate": max(left["bitrate"] or 0, right["bitrate"] or 0) or None, "quality_rank": max(left["quality_rank"], right["quality_rank"]), "native_quality": left["native_quality"] or right["native_quality"]}
    merged["quality_label"] = _quality_label(merged["width"], merged["height"], merged["bitrate"], merged["native_quality"])
    return merged


def _candidate(url: str, metadata: dict[str, Any] | None, quality_rank: int, native_quality: str | None) -> dict[str, Any]:
    metadata = metadata or {}
    width, height, bitrate = _positive(metadata.get("width")), _positive(metadata.get("height")), _positive(metadata.get("bitrate"))
    return {"url": url, "width": width, "height": height, "bitrate": bitrate, "quality_rank": quality_rank, "native_quality": native_quality, "quality_label": _quality_label(width, height, bitrate, native_quality)}


def _rank(value: dict[str, Any]) -> tuple[int, int, int, str]:
    return (-((value["width"] or 0) * (value["height"] or 0)), -(value["bitrate"] or 0), -value["quality_rank"], _media_identity(value["url"]))
...
def _child_text(element: ET.Element, name: str) -> str | None:
    return next((child.text for child in element if _local(child.tag) == name and isinstance(child.text, str)), None)
...
def _duration_ms(video: dict[str, Any]) -> int | None:
    ...
    return int(duration if duration > 10_000 else duration * 1000) if isinstance(duration, (int, float)) and duration > 0 else None
...
def _quality_label(width: int | None, height: int | None, bitrate: int | None, native_quality: str | None) -> str:
    dimensions = f"{width}×{height}" if width and height else None
    bitrate_label = f"{bitrate / 1_000_000:.1f} Mbps" if bitrate else None
    return " · ".join(value for value in (native_quality, dimensions, bitrate_label) if value) or "MP4"


def _changed() -> ProviderError:
    return ProviderError("provider_response_changed", "Facebook returned unsupported public video metadata.", 502, {"provider": "facebook"})
